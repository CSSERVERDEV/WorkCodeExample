...
'''
Python3中对List内的自定义类的实例按其属性进行排序
Python3中取消了__cmp__()方法
'''
...
L = [Student('Tim', 'Male', 99), Student('Bob', 'Male', 88), Student('Alice', 'Female', 99)]
L.sort(key=lambda x: x.gender, reverse=True) #利用lambda表达式对单个属性进行排序
returnTemp = []
for x in L:
    temp = {'name': x.name, 'gender': x.gender, 'score': x.score}
    returnTemp.append(temp)
returnData1 = {'code': 200, 'data': returnTemp}
print(json.dumps(returnData1))

py3_cmp = attrgetter('score', 'name', 'gender') #利用operator中的attrgetter方法对多个属性进行排序
L.sort(key=py3_cmp, reverse=True)
returnTemp.clear()
for x in L:
    temp = {'name': x.name, 'gender': x.gender, 'score': x.score}
    returnTemp.append(temp)
